...
/*
 * 1월 29일 더 연습이 필요해보인다.
 * 코드와 문제는 파악이 되지만 거의 연습하면서 외우면서 치는 상태이다.
 * 
 * 
 */
...
public class ArrayEx20_연습3 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.print("삽입하시겠습니까?");
			selectMenu = scan.nextInt();
			
			if (selectMenu == 1) {
				
				if (elementCnt == arr.length) {
					System.out.println("더 이상 삽입이 불가능합니다.");
					continue;
				}
				System.out.print("어디에 삽입하시겠습니까?");
				int idx = scan.nextInt();
				
				if (idx < 0 || idx > elementCnt) {
					System.out.println("해당 자리에는 삽입이 불가능합니다.");
					continue;
				}
