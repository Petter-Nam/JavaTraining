...
public class ObjectArrayEx08_연습1 {
	...
	public static void main(String[] args) {
		...
		Student8_1[] studentList = new Student8_1[3];
		int studentCnt = 0;
		...
		while (true) {
			...
			if (choice == 1) {
				...
				studentList[studentCnt] = new Student8_1();
				...
				if (studentList[select].subjects == null) {
					...
					studentList[select].subjects = new Subject8_1[1];
					studentList[select].subjects[0] = new Subject8_1();
					...
					Subject8_1[] temp = studentList[select].subjects;
					studentList[select].subjects = new Subject8_1[size +1];
					
					for (int i = 0; i < size; i++) {
						studentList[select].subjects[i] = temp[i];
					}
					
					studentList[select].subjects[size] = new Subject8_1();
