...
public class ObjectArrayEx10 {
	...
	public static void main(String[] args) {
		...
		int[] arr = new int[10];
		ArrayList<Integer> list = new ArrayList<Integer>(); // ArrayList의 <int> x <Integer> o
		...
		list.remove(2); // 처음부터 지우면 인덱스의 위치가 변경되어 삭제하고 싶은 데이터가 지워지지 않기때문에 뒤에서부터 지운다
		list.remove(1);
		list.remove(0);
		System.out.println(Arrays.toString(arr));
		System.out.println(list);
		...
		list.add(10); // null pointerexception
	}
