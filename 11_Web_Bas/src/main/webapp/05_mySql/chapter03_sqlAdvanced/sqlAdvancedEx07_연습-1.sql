...
SELECT 	
		...
		SUM(O.ORDER_GOODS_QTY) AS ORDER_GOODS_QTY
FROM  
		TB_PRODUCT P
JOIN   
		TB_ORDER   O
...
SELECT 
		SUM(P.PRICE * O.ORDER_GOODS_QTY + P.DELIVERY_PRICE) AS Profit
FROM  	
		TB_ORDER O
JOIN	
		TB_PRODUCT P
