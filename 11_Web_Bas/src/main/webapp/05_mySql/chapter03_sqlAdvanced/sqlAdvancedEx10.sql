...
AND		R.RENT_PERIOD = (SELECT 		MAX(RENT_PERIOD)
						 FROM  			RENT);
#(참고)
...
DELETE FROM CAR_BAK WHERE CAR_CD NOT IN (SELECT CAR_CD 
					  				 FROM   RENT);
SELECT * FROM CAR_BAK;
...
SELECT PRICE - (SELECT ROUND(AVG(PRICE))
