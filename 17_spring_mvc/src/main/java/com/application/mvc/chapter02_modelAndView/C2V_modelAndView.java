...
public class C2V_modelAndView {
	 ...
	 * 
	 * - 컨트롤러 클래스 메서드의 파라메타로 HttpServletRequest를 추가하여 사용한다. - setAttribute("속성명",
	 * 데이터) 메서드를 사용하여 데이터를 뷰로 전송 한다.
	 * 
	 */
	@GetMapping("/request") // local/request
	public String request(HttpServletRequest request) {
		...
		return "chapter02_modelAndView/c2v"; //template/chapter02_modelAndView/c2v 파일로 이동
	}
	/*
	 * 
	 * 2) Model
	 * 
	 * - 컨트롤러 클래스 메서드의 파라메타로 Model을 추가하여 사용한다. - addAttribute("속성명", 데이터) 메서드를 사용하여
	 * 데이터를 뷰로 전송 한다.
	 * 
	 */
	@GetMapping("/model") // local/request
	 ...
	 * 
	 * - setViewName("뷰 경로"); 메서드를 사용하여 뷰 경로를 지정한다. - setViewName(); 메서드 대신 생성자의 인수로
	 * 뷰 경로를 지정할 수 있다.
	 * 
	...
	public ModelAndView modelAndView() {
		...
		mv.setViewName("template/chapter02_modelAndView/c2v");
