...
public class ExportController {
	...
	@Autowired
	private SchedulerDAO schedulerDAO; // 단순 실습을 위한 DAO재사용(노근본)
	...
	public void productExport(HttpServletResponse response) {
		...
		try {
			...
			// Excel 헤더
			String[] header = {"상품ID" , "상품이름" , "가격" , "배송비" , "등록일자" , "브랜드ID"};
			Row row = sheet.createRow(0);                 // 1row를 생성한다.
			for (int i = 0; i < header.length; i++) {
			    Cell cell = row.createCell(i);			  // 1cell을 생성한다.
			    cell.setCellValue(header[i]);			  // 데이터를 대입한다.
			...
			// Excel 본문
			List<Map<String,Object>> productList = schedulerDAO.getProductList(); // database에서 데이터 조회
			int i = 1; // row index
			for (Map<String,Object> product : productList) { // 검색된 데이터수만큼 반복
				
				row = sheet.createRow(i);	// tr
				
				Cell cell = row.createCell(0); // td
				cell.setCellValue(product.get("PRODUCT_ID").toString()); // .toString() : 글자로 형변환하여 저장
				
				cell = row.createCell(1);	// td
				cell.setCellValue(product.get("PRODUCT_NM").toString());
				
				cell = row.createCell(2);	// td
				cell.setCellValue(product.get("PRICE").toString());
				
				cell = row.createCell(3);	// td
				cell.setCellValue(product.get("DELIVERY_PRICE").toString());
				
				cell = row.createCell(4);	// td
				cell.setCellValue(product.get("ENROLL_AT").toString());
				
				cell = row.createCell(5);	// td
				cell.setCellValue(product.get("BRAND_ID").toString());
				i++;
	...
	public void brandExport(HttpServletResponse response) {
		...
		try {
			...
			// Excel 파일명
			String fileName = "브랜드조회";
			
			// Excel Sheet
			Workbook workbook = new XSSFWorkbook();
			Sheet sheet = workbook.createSheet("브랜드");
			...
			for (Map<String,Object> brand : brandList) {
				...
				Cell cell = row.createCell(0); // td
				cell.setCellValue(brand.get("BRAND_NM").toString()); // .toString() : 글자로 형변환하여 저장
				
				cell = row.createCell(1);	// td
				cell.setCellValue(brand.get("ACTIVE_YN").toString());
