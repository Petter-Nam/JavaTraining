package com.application.training.board.config;
...
/*
   #패스워드 인코더 적용 예시
   1) build.gradle 파일의 의존성 추가 후에 refresh build project
...
@Configuration // 스프링 컨테이너는 @Configuration이 붙은 클래스를 구성 클래스로 인식하고 이안에서 선언된 메소드들을 통해 빈으로 등록한다.
